...
function arslanLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [ALPHA-NEXUS] ${new Date().toISOString()}: ${message}`);
}
    ...
    try {
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                ...
                return msg && msg.message ? msg.message : { conversation: 'ALPHA-NEXUS' };
            }
        });
            ...
            if (connection === 'open') {
                ...
                if (!existingSession) {
                    await conn.sendMessage(userJid, {
                        ...
                        caption: `\n╭────────────────────◇\n│✦ *ALPHA-NEXUS — CONNECTED* 🔥\n│✦ Type *${prefix}menu* to see all commands 💫\n│✦ Prefix 『 ${prefix} 』  Mode 〔${mode}〕\n╰────────────────────○\n*© Powered by ALPHA-NEXUS*`
                    });
                }
            }
            if (connection === 'close') {
            ...
            try {
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        displayName: '© ALPHA-NEXUS',
                        vcard: `BEGIN:VCARD\nVERSION:1.0\nFN:ALPHA-NEXUS BOY\nORG:ALPHA-NEXUS BOY;\nTEL;type=CELL;type=VOICE;waid=13135550002:13135550002\nEND:VCARD`,
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© ALPHA-NEXUS' } }
                    }},
        ...
        arslanLog(`ALPHA-NEXUS-MD Pair error: ${err.message}`, 'error');
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: 'ALPHA-md is running 🔥', activeSessions: activeSockets.size }));
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 ALPHA-MD — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
